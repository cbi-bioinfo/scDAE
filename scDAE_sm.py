...
# PARAMETERS
batch_size = 1316 #1303 #1414 #1461
ep_repeat = 10 # REPLACED TO PRINT FREQUENCY
prefetch_size = batch_size * 2
test_data_size = len(x_test)

learn_rate_sdae = 1e-3
learn_rate_sm = 1e-3
learn_rate_ft = 1e-3 
...
train_sdae_eps = 3000 #100 #EPOCHS FOR REPRESENTATION LEARNING
train_sm_eps = 3000 #100 #EPOCHS FOR TRAINING CLASSIFIER
train_ft_eps = 1000 #100 #EPOCHS FOR FINE-TUNING
...
#train_op_sdae = tf.train.AdamOptimizer(learning_rate=learn_rate_sdae).minimize(sdae_cost)
...
for model_num in xrange(repeated_model_num):
	...
	# SDAE, SM, FT
	#for temp_ep, meta_step, temp_ops, temp_feed_dict in zip([train_sm_eps, train_ft_eps], ["_sm", "_ft"], [sm_ops, ft_ops], [sm_feed_dict, ft_feed_dict]):	
	for temp_ep, meta_step, temp_ops, temp_feed_dict in zip([train_sdae_eps, train_sm_eps, train_ft_eps], ["_sdae", "_sm", "_ft"], [sdae_ops, sm_ops, ft_ops], [sdae_feed_dict, sm_feed_dict, ft_feed_dict]):	
		...
		for ep in xrange(temp_ep/ep_repeat):
			...
			if meta_step != "_sdae":
				...
				cur_acc, cur_pred, cur_label, cur_sm_out = sess.run([accuracy, pred, label, sm_out], feed_dict = {handle: handle_test, keep_prob: 1.0, phase: False, noise_r: 0})
				print(", Test_Accr:%.9f," % cur_acc, "Train_batch_accr:%.9f" % cur_accuracy, end='')

				# STORE MAX MODEL
				if max_accr < float(cur_accuracy):
					max_sm_out = cur_sm_out
				#	max_accr = cur_acc
					max_pred = cur_pred
					max_label = cur_label
	...
	np.savetxt("./pred_m" + str(num) + "_" + str(max_accr) + "_" + str(n_features) + "_features", max_pred, fmt="%.0f", delimiter=",")
	np.savetxt("./label_m" + str(num) + "_" + str(max_accr) + "_" + str(n_features) + "_features", max_label, fmt="%.0f", delimiter=",")
	np.savetxt("./sm_m" + str(num) + "_" + str(max_accr) + "_" + str(n_features) + "_features", max_sm_out, fmt="%.4f", delimiter=",")
