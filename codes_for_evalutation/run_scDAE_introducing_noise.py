...
#raw_X = pd.read_csv(x_fileName, delimiter=",", dtype=np.float32).values
#raw_Y = pd.read_csv(y_fileName, delimiter=",", dtype=np.float32).values
#x_data, x_test, y_data, y_test = train_test_split(raw_X, raw_Y, test_size=0.3, random_state=42)
...
for model_num in xrange(repeated_model_num):
	...
	np.savetxt("./prediction_AE_noise_" + str(noise_rate_input) + "_" + str(max_accr), max_pred, fmt="%.0f", delimiter=",")
	np.savetxt("./label_AE_noise_" + str(noise_rate_input) + "_" + str(max_accr), max_label, fmt="%.0f", delimiter=",")
	max_accr= 0.0
